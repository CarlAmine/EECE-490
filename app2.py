...
def format_nutrition(nutrition_data):
    cleaned = re.sub(r'[^\d.,]', '', str(nutrition_data))
    numbers = [float(s) for s in cleaned.split(',') if s.replace('.', '').isdigit()]
    labels = ['Calories', 'Total Fat (PDV)', 'Sugar (PDV)', 
             'Sodium (PDV)', 'Protein (PDV)', 'Saturated Fat (PDV)']
    return {labels[i]: numbers[i] for i in range(min(len(numbers), 6))}

def get_recipe_attributes(name):
    target = name.lower()
    matching_indices = [idx for idx, recipe_name in recipe_dict['name'].items() if target in recipe_name.lower()]
    if not matching_indices: return {"error": "No recipe found"}
    idx = matching_indices[0]
    return {
        'minutes': recipe_dict['minutes'][idx],
        'ingredients': recipe_dict['ingredients'][idx],
        'steps': recipe_dict['steps'][idx],
        'nutrition': format_nutrition(recipe_dict['nutrition'][idx])
    }

def format_ingredients(raw_ingredients):
    return [ing.strip(" '\"") for ing in ''.join(raw_ingredients).split(',') if ing.strip()]
...
def load_resources():
    # Load recipe data
    recipe_file_id = '1lE6zl-9dJKUnNu6CDuBvHPpGodKrGnoX'
    recipe_dict = np.load(gdown.download(f'https://drive.google.com/uc?id={recipe_file_id}', quiet=True), allow_pickle=True).item()
    
    # Load ML model
    model_file_id = '1tR6_8S-yISRKZR2QJ6BjU3A3V5HHUCm7'
    model_data = joblib.load(gdown.download(f'https://drive.google.com/uc?id={model_file_id}', quiet=True))
    
    # Load image model
    image_model_url = "https://github.com/CarlAmine/EECE-490/releases/download/v1.0/490Image.pkl"
    svc_model = pickle.load(requests.get(image_model_url, stream=True).raw)
    
    return recipe_dict, model_data, svc_model

recipe_dict, model_data, svc_model = load_resources()
category_dict = np.load('category_dict.npy', allow_pickle=True).item()
...
# 5. MAIN APP LAYOUT
# -------------------------------
st.title("🍳 NourishAI - Smart Recipe Generator")
...
with col2:
    ...
    if st.session_state.get('generate_clicked'):
        with st.spinner("Analyzing ingredients..."):
            # ML Model Prediction
            X_input = model_data["vectorizer"].transform([ingredients]).toarray()
            y_pred = model_data["model"].predict(X_input)
            top_recipes = model_data["label_encoder"].inverse_transform(np.argsort(y_pred, axis=1)[0][-3:][::-1])
            
            # Display ML Recommendations
            st.subheader("🍽 Recommended Recipes")
            for recipe in top_recipes:
                with st.expander(f"📌 {recipe}"):
                    details = get_recipe_attributes(recipe)
                    if 'error' not in details:
                        st.markdown(f"""
                        🕒 **Cooking Time:** {details['minutes']} minutes  
                        🥦 **Ingredients:** {', '.join(format_ingredients(details['ingredients']))}  
                        📝 **Steps:**  
                        {''.join(details['steps']).replace("', '", '\n')}
                        """)
                    else:
    ...
    if uploaded_file:
        with st.spinner("Analyzing image..."):
            img_array = preprocess_for_svc(uploaded_file.read())
            prediction = svc_model.predict(img_array)
            dish = category_dict[prediction[0]].replace('_', ' ')
            
            st.subheader(f"📸 Identified Dish: {dish}")
            details = get_recipe_attributes(dish)
            if 'error' not in details:
                with st.expander("View Recipe Details"):
                    st.markdown(f"""
                    🕒 **Cooking Time:** {details['minutes']} minutes  
                    🥗 **Nutrition:**  
                    {json.dumps(details['nutrition'], indent=2)}  
                    📝 **Steps:**  
                    {''.join(details['steps']).replace("', '", '\n')}
                    """)
